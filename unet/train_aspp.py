...
from network.unet_aspp import UNetASPP
...
def create_model(num_classes = 1, type ='vgg16', is_deconv=False):
    if type == 'vgg16':
        ...
        print('create gateasppconv model')
        model = UNetASPP(num_classes=num_classes, pretrained=True, is_deconv=is_deconv)
    else:
        assert False
    model.eval()
    return model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, default='/home/wj/local/crack_segmentation/unet/checkpoints/aspp/initial', help='output dataset directory')
    parser.add_argument('--model_type', type=str, required=False, default='aspp', choices=['vgg16', 'vgg16V2', 'vgg16V3', 'aspp'])
    parser.add_argument('--snapshot', type=str, default="/home/wj/local/crack_segmentation/unet/checkpoints/aspp/initial/model_best.pt")
    ...
    train_loader = torch.utils.data.DataLoader(train_Dataset, args.batch_size, shuffle=True, pin_memory=torch.cuda.is_available(), 
    ...
    long_id = 'unetaspp_%s_%s' % (str(args.lr), datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S'))
