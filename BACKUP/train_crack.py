...
def train(dataset, model, criterion, optimizer, validation, args, logger):
    ...
    valid_loader = torch.utils.data.DataLoader(valid_dataset, 2, shuffle=False, pin_memory=torch.cuda.is_available(), num_workers=args.num_workers)
...
if __name__ == '__main__':
    ...
    dataset = CrackDataSet(img_dir=TRAIN_IMG, img_fnames=train_img_names, img_transform=train_tfms, mask_dir=TRAIN_MASK, mask_fnames=train_mask_names, mask_transform=mask_tfms)
    ...
    criterion, criterion_val = loss_crack.get_loss(args)
    model = network.get_net(args, criterion)
    optim, scheduler = optimizer.get_optimizer(args, model)

    long_id = 'crackls315_%s_%s' % (str(args.lr), datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S'))
    logger = BoardLogger(long_id)

    train(_dataset, model, criterion, optim, validate, args, logger)
    ...
    predict(test_loader, model, latest_model_path, save_dir='./result/crackls315/test_loader', device=device)
