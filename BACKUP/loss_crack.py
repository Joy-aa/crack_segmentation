...
def get_loss(args):
    ...
    if args.img_wt_loss:
        criterion = ImageBasedCrossEntropyLoss2d(
            classes=args.num_classes, size_average=True,
            upper_bound=args.wt_bound).cuda()
    elif args.joint_edgeseg_loss:
        criterion = JointEdgeSegLoss(classes=args.num_classes, upper_bound=args.wt_bound,
    ...
    criterion_val = JointEdgeSegLoss(classes=args.num_classes, mode='val',
...
class JointEdgeSegLoss(nn.Module):
    def __init__(self, classes, weight=None, reduction='mean', ignore_index=255,
        ...
        self.seg_loss = ImageBasedCrossEntropyLoss2d().cuda()
        # if mode == 'train':
    ...
    def edge_attention(self, input, target, edge):
        ...
        indices = torch.where(edge.max(1)[0] > 0.8)
        filler[indices] = target[indices]
        return self.seg_loss(input, filler)
...
class ImageBasedCrossEntropyLoss2d(nn.Module):
    ...
    def forward(self, inputs, targets):
        output_loss = self.mask_loss(inputs.view(-1, 1), targets.view(-1, 1))
        return output_loss
