...
class TransMUNet(nn.Module):
    ...
    def __init__(self, n_classes=2, dims = [144, 240, 320]):
        super().__init__()
        ...
        self.boundary = nn.Sequential(DSConv(
            32,
            32,
            9,
            1,
    ...
    def forward(self, x, istrain=False):
        ...
        stage2 = stages[f"layer_2"]
        stages[f"layer_2"] = stage2 + F.interpolate(B_out.repeat_interleave(int(stage2.shape[1]), dim=1), size=stage2.size()[2:], mode='bilinear', align_corners=True)

        stage3 = stages[f"layer_3"]
        stages[f"layer_3"] = stage3 + F.interpolate(B_out.repeat_interleave(int(stage3.shape[1]), dim=1), size=stage3.size()[2:], mode='bilinear', align_corners=True) 

        stage4 = stages[f"layer_4"]
        stages[f"layer_4"] = stage4 + F.interpolate(B_out.repeat_interleave(int(stage4.shape[1]), dim=1), size=stage4.size()[2:], mode='bilinear', align_corners=True) 
