...
class DSConv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph,
        ...
        # self.encoder_layer = nn.TransformerEncoderLayer(d_model=in_ch, nhead=1, batch_first=True)
        # self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=1)
        # self.offset_conv = nn.Conv2d(in_ch, 2 * kernel_size, kernel_size=3, padding=1)
    ...
    def forward(self, f):
        #offset torch.Size([4,18,400,400])
        offset = self.offset_conv(f)
        # H = f.shape[2]
        # input = rearrange(f, 'b c h w -> b (h w) c')
        # offset = self.offset_conv(rearrange(self.transformer_encoder(input), 'b (h w) c -> b c h w', h = H))
        ...
        if self.morph == 0:
            #deformed_feature torch.Size([4, 16, 3600, 400])
        ...
        if self.morph == 0:
            ...
            if if_offset:
                ...
                for index in range(1, center):
                    #学习到的偏移是相对于前一个点的，需要的是相对于中心点的，所以要加上前一个点相对于中心点的偏移
