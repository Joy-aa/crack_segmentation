...
class OnlineTransformDataset1(Dataset):
    """
    Method 0 - FSS style (class/1.jpg class/1.png)
    Method 1 - Others style (XXX.jpg XXX.png)
    """
    def __init__(self, root, root_mask, root_seg, need_name=False, method=0, perturb=True):
        self.root = root
        self.gt_root = root_mask
        self.seg_root = root_seg
        self.need_name = need_name
        self.method = method
        # self.im_list = []
        ...
        if method == 0:
            ...
            self.im_list = [path.join(self.root, im) for im in os.listdir(self.root) if '.png' in im]
            self.gt_list = [path.join(self.gt_root, im) for im in os.listdir(self.gt_root) if '.png' in im]
            self.seg_list = [path.join(self.seg_root, im) for im in os.listdir(self.seg_root) if '.png' in im]
    ...
    def __getitem__(self, idx):
        ...
        if self.method == 0:
            gt = Image.open(self.gt_list[idx][:-3]+'bmp').convert('L')
            seg = Image.open(self.seg_list[idx][:-3]+'bmp').convert('L')
        else:
            gt = Image.open(self.gt_list[idx]).convert('L')
            seg = Image.open(self.seg_list[idx]).convert('L')
        ...
        reseed(seed)
        seg = self.bilinear_dual_transform(seg)
